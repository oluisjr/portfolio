...
/* ─── Preencha seus artigos aqui ─── */
const articles = [
  {
    id: '01',
    year: '2024',
    area: 'Data Science',
    title: 'Título do Seu Artigo Principal Aqui',
    titleItalic: 'Artigo Principal',
    abstract: 'Escreva aqui o resumo do seu artigo. Explique brevemente o problema abordado, a metodologia utilizada e os principais resultados ou contribuições do trabalho para a área.',
    link: '#',       // substitua pelo DOI ou URL real
    featured: true,
  },
  {
    id: '02',
    year: '2023',
    area: 'Automação',
    title: 'Segundo Artigo Científico',
    titleItalic: 'Científico',
    abstract: 'Resumo do segundo artigo. Descreva o contexto, a abordagem metodológica e os resultados obtidos neste trabalho de pesquisa.',
    link: '#',
    featured: false,
  },
  {
    ...
    year: '2023',
    area: 'Power Platform',
    title: 'Terceiro Trabalho Acadêmico',
    titleItalic: 'Acadêmico',
    abstract: 'Resumo do terceiro artigo ou trabalho acadêmico publicado. Inclua os principais pontos e contribuições.',
    link: '#',
    featured: false,
  },
]
...
function FeaturedArticle({ a }: { a: typeof articles[0] }) {
  ...
  return (
    <div ref={ref} style={{ opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(36px)', transition: 'all .85s var(--ease-expo)' }}>
      <a href={a.link} style={{ display: 'block', textDecoration: 'none', borderRadius: 'var(--r-xl)', overflow: 'hidden', border: `1px solid ${hov ? 'rgba(200,255,0,.25)' : 'var(--border-md)'}`, background: 'var(--bg-card)', transition: 'border-color .4s, box-shadow .4s', boxShadow: hov ? '0 32px 80px rgba(0,0,0,.6)' : '0 8px 32px rgba(0,0,0,.35)', cursor: 'none' }}
        ...
        <div style={{ padding: '3.5rem 4rem' }}>
          ...
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.9rem, 3.5vw, 3rem)', fontWeight: 900, lineHeight: 1.05, letterSpacing: '-.02em', color: 'var(--white)', marginBottom: '1.75rem', maxWidth: 760 }}>
            ...
            <em style={{ fontStyle: 'italic', color: hov ? 'var(--lime)' : 'var(--white-60)', fontWeight: 400, transition: 'color .3s' }}>
              {a.titleItalic}
            </em>
          </h3>
...
function ArticleRow({ a, index, total }: { a: typeof articles[0]; index: number; total: number }) {
  ...
  return (
    <div ref={ref} style={{ opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(28px)', transition: `all .7s var(--ease-expo) ${index * .1}s` }}>
      <a href={a.link} style={{ display: 'grid', gridTemplateColumns: '56px 1fr auto', gap: '2.5rem', alignItems: 'start', padding: '2.5rem 0', borderBottom: index < total - 1 ? '1px solid var(--border)' : 'none', textDecoration: 'none', cursor: 'none', transition: 'background .2s', borderRadius: 'var(--r-sm)' }}
        ...
        <div style={{ paddingTop: '.25rem' }}>
          <span style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: '1.4rem', color: hov ? 'var(--lime)' : 'var(--gray)', transition: 'color .25s' }}>
            {a.id}
          </span>
        </div>
