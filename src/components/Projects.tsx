...
const projects = [
  {
    id: '01', year: '2025', accent: '#06b6d4', featured: true,
    category: 'Next.js · TypeScript · Gestão',
    title: 'SmartPass',
    titleItalic: 'Digital Attendance',
    desc: 'Plataforma web de presença digital desenvolvida para o time de Gente e Gestão. Integração com mapas, interface responsiva e deploy em produção.',
    descShort: 'App de presença digital em Next.js.',
    tech: ['Next.js', 'React', 'TypeScript', 'Tailwind'],
    link: 'https://smartpass-lake.vercel.app/',
    img: '/smartpass.png' as string | null,
    bars: [40, 70, 55, 85, 60, 75, 50, 80, 45],
    achievements: [
      { icon: '🐍', title: 'Python Liberado', desc: 'Download de Python puro autorizado em Porto Real' },
      { icon: '🔗', title: 'Proxy Exclusivo', desc: 'Implementação de proxy dedicado para config de libs' },
      { icon: '🖥️', title: 'IIS WebApp Server', desc: 'Servidor de aplicação criado pra hospedar webapps' },
      { icon: '⚙️', title: 'Dev Server Python', desc: 'Servidor de desenvolvimento de Python autorizado' },
    ],
  },
  {
    id: '02', year: '2025', accent: '#7c3aed', featured: false,
  ...
  {
    id: '03', year: '2024', accent: '#f97316', featured: false,
    category: 'Streamlit · Python',
    title: 'Portal Controle',
    titleItalic: 'Estoque',
  ...
  {
    id: '04', year: '2024', accent: '#06b6d4', featured: false,
    category: 'Streamlit · Python',
    title: 'Portal',
    titleItalic: 'Treinamentos',
  ...
  {
    id: '05', year: '2024', accent: '#7c3aed', featured: false,
  ...
  {
    id: '06', year: '2024', accent: '#8b5cf6', featured: false,
  ...
  {
    id: '07', year: '2024', accent: '#ec4899', featured: false,
  ...
  {
    id: '08', year: '2024', accent: '#06b6d4', featured: false,
    category: 'Power BI · Teams',
    title: 'Rendimento',
    titleItalic: 'Metálico',
...
function FeaturedCard({ p }: { p: typeof projects[0] }) {
  ...
  return (
    <div ref={ref} style={{ marginBottom: '3rem', opacity: vis ? 1 : 0, transform: vis ? 'none' : 'translateY(36px)', transition: 'all .85s var(--ease-expo)' }}>
      {/* Header + Image Section */}
      <div className="dash-card detail-grid" onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', borderColor: hov ? `${p.accent}35` : undefined, marginBottom: '2rem' }}>
        {p.img
      ...
      {'achievements' in p && p.achievements && (
        <div style={{ padding: '2.5rem', background: `linear-gradient(135deg, ${p.accent}08, transparent)`, border: `1px solid ${p.accent}15`, borderRadius: 'var(--r-card)', marginTop: '1.5rem' }}>
          <h4 style={{ fontFamily: 'var(--font-mono)', fontSize: '.65rem', letterSpacing: '.18em', color: p.accent, textTransform: 'uppercase', marginBottom: '1.75rem' }}>🎯 Conquistas & Impacto</h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
            {p.achievements.map((ach, i) => (
              <div key={i} style={{ paddingLeft: '3.5rem', position: 'relative' }}>
                <div style={{ position: 'absolute', left: 0, top: 0, fontSize: '1.8rem' }}>{ach.icon}</div>
                <h5 style={{ fontFamily: 'var(--font-body)', fontSize: '.95rem', fontWeight: 600, color: 'var(--white)', marginBottom: '.4rem' }}>{ach.title}</h5>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '.82rem', color: 'var(--gray-light)', lineHeight: 1.4 }}>{ach.desc}</p>
              </div>
            ))}
          </div>
        </div>
...
function SmallCard({ p, delay = 0 }: { p: typeof projects[0]; delay?: number }) {
  ...
  return (
    <div ref={ref} className="glass glass-hover" onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ 
        opacity: vis ? 1 : 0, 
        transform: vis ? (hov ? 'translateY(-5px)' : 'none') : 'translateY(36px)', 
        transition: `all .7s var(--ease-expo) ${delay}s`, 
        borderColor: hov ? `${p.accent}28` : undefined,
        position: 'relative',
        overflow: 'hidden',
      }}>
      ...
      <div style={{
        position: 'absolute',
        inset: 0,
        background: `linear-gradient(135deg, ${p.accent}08, ${p.accent}02)`,
        opacity: hov ? 1 : 0,
      ...
      <div style={{ position: 'relative', zIndex: 2 }}>
        {p.img
          ? <div style={{ position: 'relative', overflow: 'hidden', height: 180 }}>
              ...
              <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(to bottom, transparent 50%, ${p.accent}15)`, pointerEvents: 'none' }} />
            </div>
          : <MockWin p={p} h={180} />}
        <div style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '.65rem' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '.56rem', letterSpacing: '.14em', color: p.accent, textTransform: 'uppercase', background: `${p.accent}15`, padding: '.2rem .5rem', borderRadius: '3px' }}>{p.category}</span>
            <span className="mono-sm">{p.year}</span>
          </div>
          ...
          <p className="body-md" style={{ fontSize: '.82rem', marginBottom: '1.25rem', color: 'rgba(255,255,255,.7)' }}>{p.descShort}</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.3rem', marginBottom: '1.25rem' }}>
            {p.tech.map(t => <span key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: '.52rem', letterSpacing: '.08em', color: p.accent, border: `1px solid ${p.accent}30`, padding: '.2rem .55rem', borderRadius: '3px', textTransform: 'uppercase', background: `${p.accent}08` }}>{t}</span>)}
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1.1rem', borderTop: `1px solid ${p.accent}15` }}>
            {isInternal
              ...
              : <a href={p.link} target="_blank" rel="noopener noreferrer"
                  style={{ fontFamily: 'var(--font-mono)', fontSize: '.58rem', letterSpacing: '.14em', textTransform: 'uppercase', color: p.accent, textDecoration: 'none', transition: 'letter-spacing .25s', display: 'inline-flex', alignItems: 'center', gap: '.4rem' }}
                  onMouseEnter={e => (e.currentTarget.style.letterSpacing = '.22em')}
                  onMouseLeave={e => (e.currentTarget.style.letterSpacing = '.14em')}>Ver <span style={{ fontSize: '.7rem' }}>→</span></a>
            }
            <span style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontSize: '2rem', color: `${p.accent}15`, userSelect: 'none' }}>{p.id}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <section id="projects" style={{ background: 'var(--bg)', padding: 'clamp(6rem,11vw,11rem) clamp(1rem,4vw,4rem) clamp(5rem,10vw,10rem)' }}>
      <div ref={headRef} style={{ marginBottom: '4rem' }}>
        <span className="s-tag" style={{ opacity: headIn ? 1 : 0, transition: 'opacity .5s' }}>04 — Trabalhos</span>
      ...
      <div style={{ marginTop: '5rem', position: 'relative', paddingLeft: 'clamp(1.5rem, 4vw, 3rem)', '--accent': regularProjects[0]?.accent } as React.CSSProperties}>
        <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '2px', background: 'linear-gradient(to bottom, #06b6d4, transparent)', opacity: .15 }} />
        {regularProjects.map((p, index) => (
          <div key={p.id} style={{ marginBottom: '2.5rem', marginLeft: '2rem', display: 'flex', gap: '2rem', alignItems: 'flex-start' }}>
            <div style={{ minWidth: 'fit-content', paddingTop: '.35rem' }}>
              <div style={{ width: '12px', height: '12px', borderRadius: '50%', background: p.accent, boxShadow: `0 0 12px ${p.accent}60`, position: 'relative', left: '-7px' }} />
            </div>
            <SmallCard p={p} delay={index * .08} />
          </div>
        ))}
      </div>
    </section>
